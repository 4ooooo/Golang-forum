...
func (user *User) Create() (err error) {
	...
	statement := "insert into users (uuid, name, email, password, created_at, sfz) values (?,?,?,?,?,?)"
	stmtin, err := Db.Prepare(statement)
	if err != nil {
		return
	}
	...
	uuid := createUUID()
	stmtin.Exec(uuid, user.Name, user.Email, Encrypt(user.Password), time.Now(), user.Sfz)
...
func UserByEmail(email string) (user User, err error) {
	user = User{}
	err = Db.QueryRow("SELECT id, uuid, name, email, password, created_at , sfz FROM users WHERE email = ?", email).
		Scan(&user.Id, &user.Uuid, &user.Name, &user.Email, &user.Password, &user.CreatedAt, &user.Sfz)
	return
}
...
// Create a new thread
func (user *User) CreateThread(topic string, classification string) (conv Thread, err error) {
	statement := "insert into threads (uuid, topic, user_id, created_at, num, classification) values (?, ?, ?, ?, ?, ?)"
	stmtin, err := Db.Prepare(statement)
	if err != nil {
		return
	}
	...
	time.Local = cst
	stmtin.Exec(uuid, topic, user.Id, time.Now().In(cst), 0, classification)
